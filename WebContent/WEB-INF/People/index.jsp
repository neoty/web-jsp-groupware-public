...
<div id="people-main">
			...
			<select name="to" id="to" onchange="toMethod(this.value);" class="people-combo-form" style="width: 130px;">	
					...
					<c:if test="${param.array != null}">
						<c:if test="${param.array == ''}">
							<option value="" selected>부서보기 - 전체보기</option>
						</c:if>
						<c:if test="${param.array != ''}">
							<option value="" selected>부서보기 - ${depname}</option>
						</c:if>
						<option value="">전체보기</option>
					</c:if>
					<c:forEach var="n" items="${dep_list}">
						<option value="${n.dep_no}-${n.dep_name}">${n.dep_name}</option>
					</c:forEach>
			</select>		
			</div>
	</div>
		...
		<c:forEach var="n" items="${memberList}">
			<div id="people-content">
				...
				<c:if test="${sessionScope.MB_NO == n.mb_no}">
					<div class="people-nameandposi">
						<a href="#" style="color: #2B3A42;"><b>${n.posi_name} ${n.mb_name}</b></a>
					</div>
				</c:if>
				<c:if test="${sessionScope.MB_NO != n.mb_no}">
					<div class="people-nameandposi">
						<a href="${ctx}/Chat/Write.chat?no=${n.mb_no}" style="color: #2B3A42;"><b>${n.posi_name} ${n.mb_name}</b></a>
					</div>
				</c:if>
	...
	<div id="paging">
		<div class="paginate_simple">
			<c:if test="${page > 11}">
				<a href="${ctx}/People/index.people?type=${param.type}&code=${param.code}&page=${page-10}" class="direction"> ‹ <span>Prev</span></a>
			</c:if>
			<c:forEach var="i" step="1" begin="0" end="9">
				<c:if test="${start+i <= end}">
					...
					<c:if test="${start+i != page}">
						<a href="${ctx}/People/index.people?type=${param.type}&code=${param.code}&page=${start+i}">${start+i}</a>
					</c:if>
				</c:if>
			</c:forEach>
			<c:if test="${start+10 < end}">
				<a href="${ctx}/People/index.people?type=${param.type}&code=${param.code}&page=${start+10}" class="direction"><span>Next</span> › </a>
			</c:if>
		</div>
	</div>
</div>
...
<script>
	function toMethod(value) {
		...
		var url = "${ctx}/People/index.people?array="+value;
		location.href =  url;
	}
</script>